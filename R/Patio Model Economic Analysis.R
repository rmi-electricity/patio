...
allocated_parquet <- allocated_parquet %>%
  ...
  mutate(
    ...
    across(
      c(redispatch_mwh:implied_need_mwh, redispatch_mmbtu:redispatch_cost_startup),
      ~ .x * capacity_frac
    ),
...
trans_costs <- as.numeric(cloud$read_cloud_file("patio-data/20241031/tx_dollars_per_mw_total.csv") %>% select(c(dollars_per_capacity_difference)))
...
forward_rates_name <- paste(cache_dir, paste("forward_rates-", cur_year,
...
core_metric_parameter_tech_columns <-
  c(
    "CAPEX", "CF", "Fixed O&M", "Variable O&M", "Fuel", "OCC", "Additional OCC",
...
baseline_CF <- nrel_atb_tech_intermediate %>%
  ...
  select(-c(
    CAPEX, core_metric_variable, `Fixed O&M`, `Variable O&M`, Fuel,
    `Heat Rate Penalty`
  )) %>%
  rename(base_CF = CF) %>%
...
#   if (!dir.exists(uvresultsdir)) {
#     dir.create(uvresultsdir)
#   }
#   setwd(uvresultsdir)
# } else {
#   "set file path for user"
...
master_unit_list <- read_parquet(paste(cache_dir, "unit_financial_inputs.parquet", sep = "/")) %>% mutate(plant_id_eia = as.integer64(plant_id_eia))
asset_owners <- read_parquet(paste(cache_dir, "asset_owners.parquet", sep = "/"))
...
unique_existing_assets <- full_parquet %>%
  ...
  mutate(capex_per_kw = coalesce(capex_per_kw, median(capex_per_kw, na.rm = TRUE))) %>%
  ungroup() %>%
  group_by(Technology_FERC, w_maint_ntile) %>%
  mutate(real_maint_capex_per_kw = coalesce(real_maint_capex_per_kw, median(real_maint_capex_per_kw, na.rm = TRUE))) %>%
  ungroup() %>%
...
write_parquet(owned_co2_reductions, paste(cache_dir, "owned_co2_reductions.parquet", sep = "/"))
# write_parquet(scenario_co2_reductions,"scenario_co2_reductions.parquet")
write_parquet(allocated_parquet, paste(cache_dir, "allocated_parquet.parquet", sep = "/"))
...
full_parquet_MUL <- full_parquet_MUL %>%
  ...
  ungroup() %>%
  select(-c(owned_real_maint_capex, owned_maint_capex, owned_start_depr_reserve))
...
write_parquet(refinancing_parquet, paste(cache_dir, "refinancing.parquet", sep = "/"))
...
refinancing_parquet <- read_parquet(paste(cache_dir, "refinancing.parquet", sep = "/"))
...
final_fossil_costs_by_utility_ba <- fossil_parquet %>%
  ...
  summarize(
    across(c(
      Average_Emissions, Average_Emissions_Reductions, Emissions, Emissions_Reduced, MW, MWh, MWh_no_curt,
      Fuel_Costs, VOM, FOM, Startup_Costs, Capex, Capex_Costs, Opex, Costs, Earnings, Forward_Earnings
    ), ~ sum(., na.rm = TRUE))
  ) %>%
  ungroup()
...
write_parquet(final_fossil_costs_by_utility_ba, paste(cache_dir, "final_fossil_costs_by_utility_ba.parquet", sep = "/"))
write_parquet(final_refinancing_costs_by_utility_ba, paste(cache_dir, "final_refinancing_costs_by_utility_ba.parquet", sep = "/"))
...
allocated_parquet <- read_parquet(paste(cache_dir, "allocated_parquet.parquet", sep = "/"))
...
allocated_parquet_MUL <- allocated_parquet %>%
  ...
  summarize(
    across(c(
      capacity_mw, redispatch_mwh, redispatch_mmbtu, redispatch_mmbtu, redispatch_co2_tonne, redispatch_cost_fuel,
      co2_reduced, redispatch_cost_startup, implied_need_mw
    ), ~ sum(. * fraction_owned, na.rm = TRUE), .names = "owned_{.col}"),
...
transmission_lines_parquet <- allocated_parquet %>%
  ...
  left_join(MUL_ownership %>% select(
    ...
    fraction_owned
  ) %>% distinct(), relationship = "many-to-many", by = c("plant_id_eia", "generator_id", "ba_code")) %>%
  group_by(
    Utility_ID,
    Utility_ID_Econ,
    entity_type,
  ...
  summarize(owned_capacity_mw = sum(fraction_owned * distance * capacity_mw * 0.621371, na.rm = TRUE)) %>%
  ungroup() %>%
  mutate(
    # re_plant_id = NA,
...
proposed_transmission_upgrades_parquet <- allocated_parquet %>%
  ...
  summarize(owned_capacity_mw = sum(capacity_mw * fraction_owned, na.rm = TRUE)) %>%
  ungroup() %>%
  mutate(
...
write_parquet(consolidated_final_data_inputs, paste(cache_dir, "consolidated_final_data_inputs.parquet", sep = "/"))
...
consolidated_final_data_inputs <- read_parquet(paste(cache_dir, "consolidated_final_data_inputs.parquet", sep = "/"))
fossil_parquet <- read_parquet(paste(cache_dir, "fossil.parquet", sep = "/"))
refinancing_parquet <- read_parquet(paste(cache_dir, "refinancing.parquet", sep = "/"))

final_fossil_costs_by_utility_ba <- read_parquet(paste(cache_dir, "final_fossil_costs_by_utility_ba.parquet", sep = "/"))
...
owned_co2_reductions <- read_parquet(paste(cache_dir, "owned_co2_reductions.parquet", sep = "/"))
...
write_parquet(final_data_index, paste(cache_dir, "final_data_index.parquet", sep = "/"))
        ...
        for (New_Asset_Debt_Replaced_by_EIR in New_Asset_Debt_Replaced_by_EIR_Range) {
          for (New_Asset_Equity_Replaced_by_EIR in New_Asset_Equity_Replaced_by_EIR_Range) {
            ...
            all_utility_ba_codes <- rbind(
              consolidated_final_data_inputs %>% select(c(Utility_ID_Econ, ba_code)) %>% distinct(),
              fossil_parquet %>% select(c(Utility_ID_Econ = Utility_ID, ba_code)) %>% distinct()
            ) %>% distinct()
            ...
            for (cur_build_year in loop_years) {
              ...
              final_data_inputs_selected_years <- selectable_scenarios %>%
                select(c(
                  Utility_ID_Econ, ba_code, scenario,
              ...
              utility_ba_co2_reductions <- owned_co2_reductions %>%
                ...
                group_by(Utility_ID_Econ, ba_code) %>%
                mutate(co2_red_selected = sum(co2_red * is_selected * (year > irp_year & year <= cur_build_year), na.rm = TRUE)) %>%
                ungroup() %>%
                filter(is.na(is_selected)) %>%
                group_by(Utility_ID_Econ, ba_code, scenario) %>%
                mutate(co2_red = sum(co2_red * (year > irp_year & year <= irp_year + NPV_Duration), na.rm = TRUE) + co2_red_selected) %>%
                ungroup() %>%
              ...
              final_data_inputs <- final_data_inputs %>%
                mutate(
                  ...
                  Clean_EIR_OBS_Frac = case_when(
                    Clean_EIR_Off_BS == "Yes" & iou & build_year <= EIR_NewERA_fin_build_year ~ New_Asset_Debt_Replaced_by_EIR + New_Asset_Equity_Replaced_by_EIR,
                    TRUE ~ 0
                  ),
              ...
              capital_recycled_by_scenario <- final_data_inputs %>%
                ...
                summarize(Capital_Recycled = sum(Clean_CapEx * (1 - ITC), na.rm = TRUE)) %>%
                ungroup() %>%
                ...
                counterfactual_econ <- counterfactual_econ_by_tech %>%
                  ...
                  summarize(
                    ...
                    across(c(Emissions, Emissions_Reduced),
                      ~ sum(.x * (operating_year == target_build_year + 1), na.rm = TRUE),
              ...
              if (ba_scenario_selection) {
                cur_scenario_selected <- selectable_scenarios %>%
                  ...
                  inner_join(
                    final_econ %>% filter(ba_is_selected) %>%
                      filter(!counterfactual_baseline) %>%
                      select(Utility_ID_Econ, ba_code, scenario,
                ...
                cur_scenario_selected <- selectable_scenarios %>%
                  select(-c(re_energy_max, storage_li_pct_max, re_still_limits_dispatch, nuclear_scen_max, ccs_scen_max, is_selected)) %>%
                  inner_join(
                    final_econ %>% filter(is_selected) %>%
                      filter(!counterfactual_baseline) %>%
                      select(
                        Utility_ID_Econ, ba_code, scenario, is_selected,
            ...
            final_refinancing_outputs <- refi_parquet %>%
              left_join(cur_scenario_selected %>% select(c(Utility_ID_Econ, ba_code, scenario,
                final_selected = is_selected,
                re_limits_dispatch, re_energy, nuclear_scen, storage_li_pct, ccs_scen
              )), by = c(
                "Utility_ID_Econ",
                "ba_code",
                "scenario"
            ...
            if (file.exists(paste(cache_dir, "final_outputs.parquet", sep = "/"))) {
              final_outputs_agg <- read_parquet(paste(cache_dir, "final_outputs.parquet", sep = "/"))
              final_outputs_agg <- rbind(final_outputs_agg, final_outputs %>% filter(!historical_actuals))
              write_parquet(final_outputs_agg, paste(cache_dir, "final_outputs.parquet", sep = "/"))
              scenario_selected_agg <- read_parquet(paste(cache_dir, "scenario_selected.parquet", sep = "/"))
              scenario_selected_agg <- rbind(scenario_selected_agg, scenario_selected)
              write_parquet(scenario_selected_agg, paste(cache_dir, "scenario_selected.parquet", sep = "/"))
            } else {
              write_parquet(final_outputs, paste(cache_dir, "final_outputs.parquet", sep = "/"))
              write_parquet(scenario_selected, paste(cache_dir, "scenario_selected.parquet", sep = "/"))
              final_outputs_agg <- read_parquet(paste(cache_dir, "final_outputs.parquet", sep = "/"))
              scenario_selected_agg <- read_parquet(paste(cache_dir, "scenario_selected.parquet", sep = "/"))
            }
...
cloud$write_cloud_file(final_outputs, paste("patio-results", patio_results, "final_outputs.parquet", sep = "/"))
cloud$write_cloud_file(scenario_selected, paste("patio-results", patio_results, "scenario_selected.parquet", sep = "/"))

parameters <- toJSON(parameters)
cloud$write_cloud_file(parameters, paste("patio-results", patio_results, "parameters.json", sep = "/"))
