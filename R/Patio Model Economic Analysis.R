...
allocated_parquet <- results$allocated %>%
  ...
  mutate(
    ...
    NREL_Class = as.factor(if_else(
      !is.na(NREL_Class),
      paste("Class", NREL_Class, sep = ""),
      NA
    ))
  )
...
valid_ba_scens <- full_parquet %>%
  select(c(ba_code, scenario)) %>%
  distinct()

test_data_match <- full_parquet %>%
  filter(scenario == "counterfactual") %>%
  group_by(ba_code, year) %>%
  summarise(
    redispatch_curt_adj_mwh = sum(redispatch_curt_adj_mwh, na.rm = TRUE),
    redispatch_mwh = sum(redispatch_mwh, na.rm = TRUE)
  )
...
allocated_parquet <- allocated_parquet %>%
  ...
  mutate(
    capacity_change = capacity_mw -
      if_else(num_phases > 1, coalesce(lag(capacity_mw, 1), 0), 0),
    rank = as.integer(if_else(
      num_phases > 1,
      cumsum(capacity_change >= 1),
      cumsum(if_else(
        is.na(lag(re_generator_id, 1)),
        1,
        re_generator_id != lag(re_generator_id, 1)
      ))
    ))
  ) %>%
  ungroup() %>%
  ...
  mutate(capacity_change = max(capacity_change)) %>%
  ungroup() %>%
  ...
  mutate(capacity_frac = capacity_change / max(capacity_mw)) %>%
  ungroup()
...
allocated_parquet <- allocated_parquet %>%
  ...
  filter(
    is.na(re_generator_id_new) |
      ((!is.na(re_generator_id_new)) & rank >= re_generator_id_new)
  ) %>%
  mutate(
    re_generator_id_old = re_generator_id,
    technology_id = as.integer(unclass(as.factor(technology_description)) - 1),
    re_generator_id = as.integer(
      coalesce(re_generator_id_new, rank) + technology_id * 100
    ),
    capacity_frac = coalesce(capacity_change, capacity_mw) / capacity_mw,
    capacity_mw = coalesce(capacity_change, capacity_mw),
    across(
      c(
        redispatch_mwh:implied_need_mwh,
        redispatch_mmbtu:redispatch_cost_startup
      ),
      ~ .x * capacity_frac
    ),
  ...
  mutate(test = n_distinct(capacity_mw)) %>%
  ungroup()
...
full_parquet_map <- full_parquet %>%
  select(plant_id_eia, generator_id, utility_id_eia) %>%
  distinct()
...
trans_costs <- as.numeric(
  cloud$read_cloud_file("patio-data/20241031/tx_dollars_per_mw_total.csv") %>%
    select(c(dollars_per_capacity_difference))
)
...
data_start_date <- as.Date(ifelse(
  cur_day > 10,
  make_date(cur_year, cur_month, 1),
  ifelse(
    cur_month == 1,
    make_date(cur_year - 1, 12, 1),
...
if (file.exists(forward_rates_name)) {
  ...
  treasury_link_prev <- paste(
    "https://home.treasury.gov/resource-center/data-chart-center/interest-rates/daily-treasury-rates.csv/all/",
    prev_year,
    ifelse(prev_month >= 10, prev_month, 0),
    ifelse(prev_month >= 10, "", prev_month),
    "?type=daily_treasury_yield_curve&field_tdr_date_value_month=",
    prev_year,
    ifelse(prev_month >= 10, prev_month, 0),
    ifelse(prev_month >= 10, "", prev_month),
  ...
  if (cur_day > 3) {
    treasury_yields_raw <- as.data.frame(rbind(
      read.csv(treasury_link),
      read.csv(treasury_link_prev)
    ))
  } else {
  ...
  intercept <- as.numeric(
    treasury_variables %>% filter(term == "(Intercept)") %>% select(estimate)
  )
  tenor_coeff <- as.numeric(
    treasury_variables %>% filter(term == "tenor") %>% select(estimate)
  )
  tenorsq_coeff <- as.numeric(
    treasury_variables %>% filter(term == "tenorsq") %>% select(estimate)
  )

  Forward_Interest_Rates <- cross_join(
    as.data.frame(all_years),
    as.data.frame(all_tenors)
  ) %>%
    rename("Years" = "all_years", "Tenor" = "all_tenors") %>%
    mutate(
      Forward_Treasury_Rates = intercept +
        tenor_coeff * Tenor +
        tenorsq_coeff * Tenor^2,
      Spot_Rates_Issuance_Year = intercept +
        tenor_coeff * (Years - cur_year) +
        tenorsq_coeff * (Years - cur_year)^2,
      Weighted_Average_Tenor = (Tenor + 1) -
        (1 + Forward_Treasury_Rates) / Forward_Treasury_Rates +
  ...
  for (i in 1:10) {
    Forward_Interest_Rates <- Forward_Interest_Rates %>%
      mutate(
        Spot_Rates_at_Maturity = intercept +
          tenor_coeff * (Years - cur_year + Weighted_Average_Tenor) +
          tenorsq_coeff * (Years - cur_year + Weighted_Average_Tenor)^2,
        Forward_Treasury_Rates = (((1 + Spot_Rates_at_Maturity)^(Years -
          cur_year +
          Weighted_Average_Tenor)) /
          ((1 + Spot_Rates_Issuance_Year)^(Years - cur_year)))^(1 /
          Weighted_Average_Tenor) -
          1,
        Weighted_Average_Tenor = (Tenor + 1) -
          (1 + Forward_Treasury_Rates) / Forward_Treasury_Rates +
          Tenor / (((1 + Forward_Treasury_Rates)^Tenor) - 1),
        Spot_Rates_at_Maturity_DOE = intercept +
          tenor_coeff * (Years - cur_year + Weighted_Average_Tenor_DOE) +
          tenorsq_coeff * (Years - cur_year + Weighted_Average_Tenor_DOE)^2,
        DOE_Loan_Rates = 0.00375 +
          (((1 + Spot_Rates_at_Maturity_DOE)^(Years -
            cur_year +
            Weighted_Average_Tenor_DOE)) /
            ((1 + Spot_Rates_Issuance_Year)^(Years - cur_year)))^(1 /
            Weighted_Average_Tenor_DOE) -
          1,
        Weighted_Average_Tenor_DOE = (Tenor + 1) -
          (1 + DOE_Loan_Rates) / DOE_Loan_Rates +
          Tenor / (((1 + DOE_Loan_Rates)^Tenor) - 1),
        Spot_Rates_at_Maturity_BBB = intercept +
          tenor_coeff * (Years - cur_year + Weighted_Average_Tenor_BBB) +
          tenorsq_coeff * (Years - cur_year + Weighted_Average_Tenor_BBB)^2,
        BBB_Loan_Rates = BBB_Spread +
          (((1 + Spot_Rates_at_Maturity_BBB)^(Years -
            cur_year +
            Weighted_Average_Tenor_BBB)) /
            ((1 + Spot_Rates_Issuance_Year)^(Years - cur_year)))^(1 /
            Weighted_Average_Tenor_BBB) -
          1,
        Weighted_Average_Tenor_BBB = (Tenor + 1) -
          (1 + BBB_Loan_Rates) / BBB_Loan_Rates +
          Tenor / (((1 + BBB_Loan_Rates)^Tenor) - 1),
        Spot_Rates_at_Maturity_AAA = intercept +
          tenor_coeff * (Years - cur_year + Weighted_Average_Tenor_AAA) +
          tenorsq_coeff * (Years - cur_year + Weighted_Average_Tenor_AAA)^2,
        AAA_Loan_Rates = AAA_Spread +
          (((1 + Spot_Rates_at_Maturity_AAA)^(Years -
            cur_year +
            Weighted_Average_Tenor_AAA)) /
            ((1 + Spot_Rates_Issuance_Year)^(Years - cur_year)))^(1 /
            Weighted_Average_Tenor_AAA) -
          1,
        Weighted_Average_Tenor_AAA = (Tenor + 1) -
          (1 + AAA_Loan_Rates) / AAA_Loan_Rates +
          Tenor / (((1 + AAA_Loan_Rates)^Tenor) - 1),
        Spot_Rates_at_Maturity_AA = intercept +
          tenor_coeff * (Years - cur_year + Weighted_Average_Tenor_AA) +
          tenorsq_coeff * (Years - cur_year + Weighted_Average_Tenor_AA)^2,
        AA_Loan_Rates = AA_Spread +
          (((1 + Spot_Rates_at_Maturity_AA)^(Years -
            cur_year +
            Weighted_Average_Tenor_AA)) /
            ((1 + Spot_Rates_Issuance_Year)^(Years - cur_year)))^(1 /
            Weighted_Average_Tenor_AA) -
          1,
        Weighted_Average_Tenor_AA = (Tenor + 1) -
          (1 + AA_Loan_Rates) / AA_Loan_Rates +
          Tenor / (((1 + AA_Loan_Rates)^Tenor) - 1),
        Spot_Rates_at_Maturity_A = intercept +
          tenor_coeff * (Years - cur_year + Weighted_Average_Tenor_A) +
          tenorsq_coeff * (Years - cur_year + Weighted_Average_Tenor_A)^2,
        A_Loan_Rates = A_Spread +
          (((1 + Spot_Rates_at_Maturity_A)^(Years -
            cur_year +
            Weighted_Average_Tenor_A)) /
            ((1 + Spot_Rates_Issuance_Year)^(Years - cur_year)))^(1 /
            Weighted_Average_Tenor_A) -
          1,
        Weighted_Average_Tenor_A = (Tenor + 1) -
          (1 + A_Loan_Rates) / A_Loan_Rates +
          Tenor / (((1 + A_Loan_Rates)^Tenor) - 1),
        Spot_Rates_at_Maturity_BB = intercept +
          tenor_coeff * (Years - cur_year + Weighted_Average_Tenor_BB) +
          tenorsq_coeff * (Years - cur_year + Weighted_Average_Tenor_BB)^2,
        BB_Loan_Rates = BB_Spread +
          (((1 + Spot_Rates_at_Maturity_BB)^(Years -
            cur_year +
            Weighted_Average_Tenor_BB)) /
            ((1 + Spot_Rates_Issuance_Year)^(Years - cur_year)))^(1 /
            Weighted_Average_Tenor_BB) -
          1,
        Weighted_Average_Tenor_BB = (Tenor + 1) -
          (1 + BB_Loan_Rates) / BB_Loan_Rates +
          Tenor / (((1 + BB_Loan_Rates)^Tenor) - 1),
        Spot_Rates_at_Maturity_B = intercept +
          tenor_coeff * (Years - cur_year + Weighted_Average_Tenor_B) +
          tenorsq_coeff * (Years - cur_year + Weighted_Average_Tenor_B)^2,
        B_Loan_Rates = B_Spread +
          (((1 + Spot_Rates_at_Maturity_B)^(Years -
            cur_year +
            Weighted_Average_Tenor_B)) /
            ((1 + Spot_Rates_Issuance_Year)^(Years - cur_year)))^(1 /
            Weighted_Average_Tenor_B) -
          1,
        Weighted_Average_Tenor_B = (Tenor + 1) -
          (1 + B_Loan_Rates) / B_Loan_Rates +
...
irp_treas_rate <- as.numeric(
  Forward_Interest_Rates %>%
    filter(Tenor == Debt_Tenor, Years == irp_year) %>%
    select(c(Forward_Treasury_Rates))
)
irp_BBB_rate <- as.numeric(
  Forward_Interest_Rates %>%
    filter(Tenor == Debt_Tenor, Years == irp_year) %>%
    select(c(BBB_Loan_Rates))
)
irp_A_rate <- as.numeric(
  Forward_Interest_Rates %>%
    filter(Tenor == Debt_Tenor, Years == irp_year) %>%
    select(c(A_Loan_Rates))
)
irp_AA_rate <- as.numeric(
  Forward_Interest_Rates %>%
    filter(Tenor == Debt_Tenor, Years == irp_year) %>%
    select(c(AA_Loan_Rates))
)
irp_B_rate <- as.numeric(
  Forward_Interest_Rates %>%
    filter(Tenor == Debt_Tenor, Years == irp_year) %>%
    select(c(B_Loan_Rates))
)
irp_BB_rate <- as.numeric(
  Forward_Interest_Rates %>%
    filter(Tenor == Debt_Tenor, Years == irp_year) %>%
    select(c(BB_Loan_Rates))
)
irp_AAA_rate <- as.numeric(
  Forward_Interest_Rates %>%
    filter(Tenor == Debt_Tenor, Years == irp_year) %>%
    select(c(AAA_Loan_Rates))
)

technology_columns_fossil <- c(
  "Coal_FE",
  "NaturalGas_FE",
  "Coal_Retrofits",
  "NaturalGas_Retrofits"
...
nrel_atb_tech_intermediate <- nrel_atb_tech_intermediate %>%
  pivot_wider(
    names_from = core_metric_parameter,
    values_from = value
  )
...
nrel_atb_tech_intermediate <- nrel_atb_tech_intermediate %>%
  ...
  mutate(
    CAPEX = coalesce(
      CAPEX,
      OCC +
        (OCC * Construction_Finance_Adder * (Construction_Finance_Years + 1)) /
          2,
      `Additional OCC` +
        (`Additional OCC` *
          Construction_Finance_Adder *
          (Construction_Finance_Years + 1)) /
          2
    )
  ) %>%
...
nrel_atb_tech_intermediate <- nrel_atb_tech_intermediate %>%
  mutate(
    `Variable O&M` = case_when(
      is.na(`Variable O&M`) ~ 0,
      TRUE ~ `Variable O&M`
    )
  )
...
fossil_tech_combs <- nrel_atb_tech_intermediate %>%
  filter(
    technology == "Coal_Retrofits" | technology == "NaturalGas_Retrofits"
  ) %>%
  select(c(technology, techdetail)) %>%
  distinct()

nrel_atb_tech_intermediate_temp <- nrel_atb_tech_intermediate %>%
  filter(
    (technology == "Coal_FE" & techdetail == "New") |
      (technology == "NaturalGas_FE" &
        (techdetail %in% c("F-Frame CC", "H-Frame CC")))
  ) %>%
  mutate(
  ...
  left_join(
    fossil_tech_combs %>% select(c(technology, techdetail2 = techdetail)),
    by = c("technology"),
    relationship = "many-to-many"
  ) %>%
  filter(
    (grepl("F-F", techdetail2) & grepl("F-F", techdetail)) |
      (grepl("H-F", techdetail2) & grepl("H-F", techdetail)) |
      technology == "Coal_Retrofits"
  ) %>%
  select(-c(techdetail)) %>%
  rename("techdetail" = "techdetail2")

nrel_atb_tech_intermediate <- rbind(
  nrel_atb_tech_intermediate_temp,
  nrel_atb_tech_intermediate
)
...
baseline_CF <- nrel_atb_tech_intermediate %>%
  ...
  rename(base_CF = CF) %>%
  filter(!is.na(base_CF))

nrel_atb_tech_intermediate <- nrel_atb_tech_intermediate %>%
  left_join(baseline_CF, by = c("technology", "techdetail"))

nrel_atb_tech_intermediate <- nrel_atb_tech_intermediate %>%
  mutate(
    CF_improvement = if_else(is.na(base_CF) | base_CF == 0, NA, CF / base_CF)
  )
...
full_parquet <- full_parquet %>%
  ...
  mutate(
    ...
    Technology_FERC = case_when(
      grepl("Batteries", technology_description) |
        grepl("Storage", technology_description) ~
        "renewables",
...
full_proposed_parquet <- full_parquet %>%
  ...
  mutate(
    ...
    co2_reduced = if_else(
      ...
      sum(
        if_else(scenario == "counterfactual", redispatch_co2_tonne, 0),
        na.rm = TRUE
      ) -
        redispatch_co2_tonne,
      0
...
transmission_upgrades_patio_parquet <- full_parquet %>%
  ...
  mutate(capacity_mw = max(capacity_mw, na.rm = TRUE)) %>%
  ungroup() %>%
  filter(capacity_mw > 0)
...
allocated_parquet <- allocated_parquet %>%
  mutate(
      ...
      as.factor(
        case_when(
          (technology_description == "Conventional Steam Coal") &
            ccs_eligible &
            ccs_scen != 0 ~
            paste(CCS_CO2_Frac * 100, "%-CCS", sep = ""),
          (technology_description == "Natural Gas Fired Combined Cycle") &
            ccs_eligible &
            ccs_scen != 0 &
            operational_year <= 2015 ~
            paste("F-Frame CC ", CCS_CO2_Frac * 100, "%-CCS", sep = ""),
          (technology_description == "Natural Gas Fired Combined Cycle") &
            ccs_eligible &
            ccs_scen != 0 &
            operational_year >= 2015 ~
            paste("H-Frame CC ", CCS_CO2_Frac * 100, "%-CCS", sep = ""),
          (technology_description == "Conventional Steam Coal") &
          ...
          (technology_description == "Natural Gas Fired Combined Cycle") &
            category == "proposed_fossil" ~
            "H-Frame CC",
          (technology_description == "Natural Gas Fired Combustion Turbine") &
            category == "proposed_fossil" ~
            "F-Frame CT",
    ...
    cf_actual = redispatch_mwh /
      (capacity_mw * (8760 + if_else(year %% 4 == 0, 24, 0)))
  ) %>%
  select(-c(technology_description)) %>%
  rename("technology_description" = "technology_NREL")
...
renewable_nrel_class <- allocated_parquet %>%
  ...
  summarize(
    cf_median = median(
      if_else(redispatch_mwh > 0 & capacity_mw > 0, cf_actual, NA),
      na.rm = TRUE
    )
  ) %>%
  left_join(
  ...
  ungroup() %>%
  filter(ren_NREL_Class == techdetail) %>%
  select(-c(techdetail))
...
allocated_parquet <- allocated_parquet %>%
  ...
  mutate(NREL_Class = as.factor(coalesce(NREL_Class, ren_NREL_Class))) %>%
  select(-c(ren_NREL_Class))
...
MUL_ownership <- full_parquet %>%
  select(plant_id_eia, generator_id, ba_code) %>%
  distinct() %>%
  left_join(
    master_unit_list %>%
      select(c(
  ...
  left_join(
    Utilities %>%
      select(c(
        ...
        owner_entity_type_ER = entity_type_ER
      )),
    by = ("Utility_ID")
  ) %>%
  left_join(
    Utilities %>%
      select(c(
        ...
        td_entity_type_ER = entity_type_ER
      )),
    by = ("td_utility_id_eia")
  ) %>%
  mutate(
...
MUL_ownership <- MUL_ownership %>%
  ...
  mutate(
    ...
    td_utility_id_eia = coalesce(
      td_utility_id_eia,
      td_utility_id_eia[which.max(ba_td_capacity)]
    ),
    fraction_owned = coalesce(fraction_owned, 1),
    entity_type = as.factor(coalesce(
      entity_type,
      operator_entity_type,
      entity_type[which.max(ba_owned_capacity)]
    )),
    entity_type_TD = as.factor(coalesce(
      entity_type_TD,
      entity_type_TD[which.max(ba_td_capacity)]
    )),
    ...
    operator_entity_type = as.factor(coalesce(
      operator_entity_type,
      operator_entity_type[which.max(ba_capacity)]
    ))
  ) %>%
  ungroup() %>%
  select(
    -c(ba_owned_capacity, owned_capacity, capacity, ba_capacity, ba_td_capacity)
  ) %>%
  distinct()

if (!IRA_on) {
  asset_owners <- asset_owners %>%
    mutate(Direct_Pay = "No", Transferability = "No")
}

asset_owners <- asset_owners %>%
  mutate(
    public_utility = (entity_type == "F" |
      entity_type == "M" |
      entity_type == "P" |
      entity_type == "S"),
    federal_entity = (entity_type == "F"),
    coop = (entity_type == "C"),
    can_claim_credit = ((!public_utility) |
      Direct_Pay == "Yes" |
      Transferability == "Yes"),
    can_monetize_credit = can_claim_credit &
      (Transferability == "Yes" | Direct_Pay == "Yes"),
...
unique_existing_assets <- full_parquet %>%
  ...
  summarize(
    cf_actual = sum(cf_actual * capacity_mw, na.rm = TRUE) /
      sum(capacity_mw, na.rm = TRUE),
    capex_per_kw = sum(
      if_else(
        is.na(capex_per_kw) | (capex_per_kw <= 100),
        NA,
        capex_per_kw * capacity_mw
      ),
      na.rm = TRUE
    ) /
      sum(
        if_else(is.na(capex_per_kw) | (capex_per_kw <= 100), NA, capacity_mw),
        na.rm = TRUE
      ),
    real_maint_capex_per_kw = sum(
      if_else(
        is.na(real_maint_capex_per_kw) | (real_maint_capex_per_kw <= 2),
        NA,
        real_maint_capex_per_kw * capacity_mw
      ),
      na.rm = TRUE
    ) /
      sum(
  ...
  arrange(cf_actual) %>%
  mutate(
    real_maint_capex_per_kw = if_else(
      is.na(real_maint_capex_per_kw) | (real_maint_capex_per_kw <= 2),
      NA,
      real_maint_capex_per_kw
    ),
  ...
  mutate(
    capex_per_kw = coalesce(capex_per_kw, median(capex_per_kw, na.rm = TRUE))
  ) %>%
  ungroup() %>%
  group_by(Technology_FERC, w_maint_ntile) %>%
  mutate(
    real_maint_capex_per_kw = coalesce(
      real_maint_capex_per_kw,
      median(real_maint_capex_per_kw, na.rm = TRUE)
    )
  ) %>%
  ungroup() %>%
...
# test to make sure MWhs match with original file
test_data_match <- test_data_match %>%
  left_join(
    full_parquet %>%
      filter(scenario == "counterfactual") %>%
      group_by(ba_code, operational_year) %>%
      summarise(
        MWh_1 = sum(redispatch_mwh, na.rm = TRUE),
        MWh_no_curt_1 = sum(redispatch_curt_adj_mwh, na.rm = TRUE)
      ),
    by = c(
      "ba_code" = "ba_code",
  ...
  mutate(
    curt_adj_check_1 = case_when(
      redispatch_curt_adj_mwh / MWh_1 <= 0.99 ~ 1,
      redispatch_curt_adj_mwh / MWh_1 >= 1.01 ~ 1,
      TRUE ~ 0
    ),
    no_curt_adj_check_1 = case_when(
      redispatch_mwh / MWh_no_curt_1 <= 0.99 ~ 1,
      redispatch_mwh / MWh_no_curt_1 >= 1.01 ~ 1,
      TRUE ~ 0
    )
  )

# warning flags if mismatching MWhs
if (sum(test_data_match$curt_adj_check_1) > 0) {
  paste("ERROR:", sum(test_data_match$curt_adj_check_1), "mismatching rows")
}
if (sum(test_data_match$no_curt_adj_check_1) > 0) {
  paste("ERROR:", sum(test_data_match$curt_adj_check_1), "mismatching rows")
}
...
full_parquet_MUL <- full_parquet %>%
  ...
  mutate(
    ...
    owned_real_maint_capex = coalesce(real_maint_capex_per_kw, 0) *
      1000 *
      owned_capacity_mw
  ) %>%
  select(c(
  ...
  mutate(
    owned_capex = coalesce(
      owned_capex,
      capex_per_kw_FERC * 1000 * owned_capacity_mw
    ),
  )
...
full_parquet_MUL <- full_parquet_MUL %>%
  ...
  mutate(
    owned_co2_reduced_patio = if_else(
      owned_redispatch_co2_tonne > 0,
      sum(
        if_else(scenario == "counterfactual", owned_redispatch_co2_tonne, 0),
        na.rm = TRUE
      ) -
        owned_redispatch_co2_tonne,
      0
  ...
  mutate(
    owned_co2_reduced = if_else(
      owned_redispatch_co2_tonne > 0,
      sum(
        if_else(
          year == baseline_emissions_year & scenario == "counterfactual",
          owned_redispatch_co2_tonne,
          0
        ),
        na.rm = TRUE
...
test_data_match <- test_data_match %>%
  left_join(
    full_parquet_MUL %>%
      ...
      group_by(ba_code, year) %>%
      summarise(
        MWh_1 = sum(owned_redispatch_mwh, na.rm = TRUE),
        MWh_no_curt_1 = sum(MWh_no_curt, na.rm = TRUE)
      ),
    by = c(
      "ba_code" = "ba_code",
      "year" = "year"
  ...
  mutate(
    ...
    no_curt_adj_check_1 = case_when(
      redispatch_mwh / MWh_no_curt_1 <= 0.91 ~ 1,
      redispatch_mwh / MWh_no_curt_1 >= 1.01 ~ 1,
      TRUE ~ 0
    )
  )
...
full_parquet_MUL <- full_parquet_MUL %>%
  ...
  mutate(
    ...
    owned_maint_capex = case_when(
      is.na(retirement_year) |
        ((!is.na(retirement_year)) &
          ((year <= retirement_year) | (retirement_year > 2035))) ~
        pmax(
          owned_real_maint_capex * Inflation_Factor_2021,
          owned_capex * depreciation_rate
        ),
      ((!is.na(retirement_year)) & year <= retirement_year) ~
        owned_real_maint_capex * Inflation_Factor_2021,
      TRUE ~ 0
    ),
    owned_capex = owned_capex + cumsum(owned_maint_capex),
    across(
      c(owned_opex, Fuel_Costs, VOM, FOM, Startup_Costs),
      ~ . * Inflation_Factor_2021
    ),
    owned_depr_reserve = pmin(
      owned_capex,
      owned_start_depr_reserve + cumsum(owned_capex * depreciation_rate)
    ),
    owned_depr_expense = owned_depr_reserve -
      if_else(
        year == min(year),
        owned_start_depr_reserve,
        lag(owned_depr_reserve, 1)
      ),
    owned_depr_reserve = pmin(
      owned_capex,
      owned_start_depr_reserve + cumsum(owned_depr_expense)
    ),
    owned_depr_expense = owned_depr_reserve -
      if_else(
        year == min(year),
    ...
    owned_ratebase = owned_capex -
      (owned_depr_reserve - owned_depr_expense / 2),
    frac_depr = 1 - (owned_ratebase / owned_capex),
    depr_life = round(owned_ratebase / owned_depr_expense),
    Average_MWh = if_else(
      owned_redispatch_mwh > 0,
      mean(
        if_else(owned_redispatch_mwh > 0, owned_redispatch_mwh, NA),
        na.rm = TRUE
      ),
      0
    ),
  ...
  ungroup() %>%
  select(
    -c(owned_real_maint_capex, owned_maint_capex, owned_start_depr_reserve)
  )
...
full_parquet_MUL <- full_parquet_MUL %>%
  mutate(
    ...
    Capex_Costs = owned_depr_expense +
      owned_ratebase * adjusted_ror / (1 - Blended_Tax_Rate),
...
refinancing_parquet <- full_parquet_MUL %>%
  ...
  mutate(
    ...
    adjusted_ror = (roe * equity_ratio) +
      ((debt_cost * (1 - equity_ratio)) * (1 - Blended_Tax_Rate)),
    Securitization_Amount = (category == "existing_fossil" &
    ...
    Securitization_PMT = Securitization_Amount *
      Securitization_Rate /
      (1 - (1 / (1 + Securitization_Rate))^Debt_Tenor),
    Securitization_OC = (category == "existing_fossil" &
      build_year <= EIR_NewERA_fin_build_year) *
      owned_capex *
      case_when(
        Technology_FERC == "steam" ~ Steam_EIR_Frac,
  ...
  distinct() %>%
  cross_join(as.data.frame(max_years), copy = TRUE) %>%
  rename("cur_project_year" = "max_years") %>%
  filter(
    (cur_project_year <= pmax(depr_life, Debt_Tenor)) &
      Securitization_Amount > 0
  ) %>%
  mutate(
    ...
    refi_depr_reserve = case_when(
      (cur_project_year <= depr_life) ~
        -(Securitization_OC - Securitization_Amount) -
          cur_project_year * Securitization_Amount / depr_life,
      (cur_project_year > depr_life) ~ -Securitization_OC,
      TRUE ~ 0
    ),
    refi_capex = -Securitization_OC,
    refi_ratebase = if_else(
      cur_project_year > depr_life,
      0,
      refi_capex - (refi_depr_reserve - refi_depr_expense / 2)
    ),
    Earnings = refi_ratebase * roe * equity_ratio,
    Costs = refi_depr_expense +
      refi_ratebase * adjusted_ror / (1 - Blended_Tax_Rate) +
      if_else(
        (cur_project_year > 0) & (cur_project_year <= Debt_Tenor),
        Securitization_PMT,
        0
      ),
    Capex_Costs = Costs,
...
final_refinancing_costs_by_utility_ba <- refinancing_parquet %>%
  ...
  summarize(
    across(
      c(Securitization_Amount, Costs, Earnings, Forward_Earnings),
      ~ sum(., na.rm = TRUE)
    )
  ) %>%
  ungroup() %>%
...
test_data_match <- test_data_match %>%
  ...
  mutate(
    curt_adj_check_2 = case_when(
      redispatch_curt_adj_mwh / MWh_2 <= 0.99 ~ 1,
      redispatch_curt_adj_mwh / MWh_2 >= 1.01 ~ 1,
      is.na(redispatch_curt_adj_mwh) ~ 0,
      TRUE ~ 0
    ),
    no_curt_adj_check_2 = case_when(
      redispatch_mwh / MWh_no_curt_2 <= 0.99 ~ 1,
      redispatch_mwh / MWh_no_curt_2 >= 1.01 ~ 1,
      is.na(redispatch_mwh) ~ 0,
      TRUE ~ 0
    )
  )

# warning flags if mismatching MWhs
if (sum(test_data_match$curt_adj_check_2) > 0) {
  paste("ERROR:", sum(test_data_match$curt_adj_check_2), "mismatching rows")
}
if (sum(test_data_match$no_curt_adj_check_2) > 0) {
  paste("ERROR:", sum(test_data_match$curt_adj_check_2), "mismatching rows")
}
...
allocated_parquet_MUL <- allocated_parquet %>%
  ...
  summarize(
    across(
      ...
      ~ sum(. * fraction_owned, na.rm = TRUE),
      .names = "owned_{.col}"
    ),
    MWh_no_curt = sum(fraction_owned * MWh_no_curt, na.rm = TRUE),
    operational_year = max(operational_year),
    cf_actual = sum(cf_actual * fraction_owned * redispatch_mwh, na.rm = TRUE) /
      owned_redispatch_mwh
  ) %>%
  ungroup()
...
transmission_lines_parquet <- allocated_parquet %>%
  ...
  left_join(
    MUL_ownership %>%
      ...
      distinct(),
    relationship = "many-to-many",
    by = c("plant_id_eia", "generator_id", "ba_code")
  ) %>%
  group_by(
    Utility_ID,
    Utility_ID_Econ,
  ...
  summarize(
    owned_capacity_mw = sum(
      fraction_owned * distance * capacity_mw * 0.621371,
      na.rm = TRUE
    )
  ) %>%
  ungroup() %>%
...
proposed_transmission_upgrades_parquet <- allocated_parquet %>%
  ...
  summarize(
    owned_capacity_mw = sum(capacity_mw * fraction_owned, na.rm = TRUE)
  ) %>%
  ungroup() %>%
  mutate(
    re_plant_id = paste(
...
allocated_parquet_MUL <- rbind(
  allocated_parquet_MUL,
...
consolidated_final_data_inputs <- consolidated_final_data_inputs %>%
  ...
  mutate(
    ...
    proposed = (category %in%
      c("proposed_fossil", "proposed_clean", "proposed_transmission_upgrades"))
  ) %>%
...
Inflator_2022 <- (CPIU %>%
  subset(Year == 2022, c(Inflation_Factor_2021)) %>%
  as.numeric())
Inflator_2025 <- (CPIU %>%
  subset(Year == 2025, c(Inflation_Factor_2021)) %>%
  as.numeric())
NewERA_subsidy_cost <- as.numeric(
  NewERA_Credit_Subsidy_Table %>%
...
if (!IRA_on) {
  data_input_parameters <- data_input_parameters %>% mutate(Normalized = "Yes")
}

data_input_parameters <- data_input_parameters %>%
  mutate(
    tax_equity_used = (direct_pay_eligible) &
      (!iou) &
      (Technology_FERC != "transmission"),
    ...
    Asset_Life = if_else(
      technology_description == "Transmission",
      pmax(
        if_else(depreciation_rate > 0, round(1 / depreciation_rate), 0),
        Clean_Life
      ),
      Clean_Life
    ),
    ...
    roe = if_else(public_utility, A_Loan_Rates, roe),
    debt_cost = if_else(
      public_utility & build_year <= EIR_NewERA_fin_build_year,
      (A_Loan_Rates) * (1 - Public_EIR_Frac) + DOE_Loan_Rates * Public_EIR_Frac,
      debt_cost
    ),
    ror = if_else(public_utility, A_Loan_Rates, ror),
    equity_ratio = if_else(public_utility, Min_Pub_Equity_Ratio, equity_ratio),
    ...
    ITC_Norm_Period = case_when(
      ...
      (Acc_Norm_Public_Coop == "Yes") & (public_utility | coop) ~
        ITC_Norm_Period,
      TRUE ~ Asset_Life
    ),
    ITC_Normalized = (iou & (Normalized == "Yes")),
    ITC = (can_claim_credit) *
      (ITC + 0.1 * (Domestic_Content_Percent + energy_community)),
    ITC_claim = ITC * (cur_project_year <= ITC_Norm_Period),
    ITC_reg = ITC_claim *
      if_else(transferability_eligible, Transferability_Discount, 1),
    Clean_PTC_est = coalesce(
      -can_claim_credit *
        PTC_Year *
        CF *
...
data_input_parameters <- data_input_parameters %>%
  ...
  mutate(
    ITC_Better = (-sum(Clean_PTC_est * Disc_Fin, na.rm = TRUE) <
      ITC_reg * Clean_CapEx_est) &
      (ITC_reg > 0)
  ) %>%
  ungroup()
...
if (replace_results) {
  file.remove(paste(cache_dir, "final_outputs.parquet", sep = "/"))
  file.remove(paste(cache_dir, "scenario_selected.parquet", sep = "/"))
}
      ...
      for (ba_least_cost in ba_least_cost_range) {
        for (New_Asset_Debt_Replaced_by_EIR in New_Asset_Debt_Replaced_by_EIR_Range) {
          for (New_Asset_Equity_Replaced_by_EIR in New_Asset_Equity_Replaced_by_EIR_Range) {
            ...
            all_utility_ba_codes <- rbind(
              consolidated_final_data_inputs %>%
                select(c(Utility_ID_Econ, ba_code)) %>%
                distinct(),
              fossil_parquet %>%
                select(c(Utility_ID_Econ = Utility_ID, ba_code)) %>%
                distinct()
            ) %>%
              distinct()
            ...
            scenario_selected <- all_scenarios %>%
              cross_join(as.data.frame(early_years), copy = TRUE) %>%
              rename(build_year = early_years) %>%
              filter(
                re_energy <= pmax(0, (build_year - (irp_year - 1)) / 5)
              ) %>%
              mutate(
              ...
              mutate(
                ...
                re_still_limits_dispatch = as.logical(
                  sum(
                    if_else(is_selected, re_limits_dispatch, NA),
                    na.rm = TRUE
                  ) >
                    0
                ),
            ...
            for (cur_build_year in loop_years) {
              ...
              scenario_selected_operating <- scenario_selected %>%
                mutate(operating_year = build_year + 1) %>%
                select(-c(build_year))

              selectable_scenarios <- all_scenarios %>%
                left_join(
                  cur_scenario_selected %>%
                    select(c(
                      Utility_ID_Econ,
                      ba_code,
                      re_energy_max,
                      storage_li_pct_max,
                      re_still_limits_dispatch,
                      nuclear_scen_max,
                      ccs_scen_max
                    )) %>%
                    distinct(),
                  by = c("Utility_ID_Econ", "ba_code")
                ) %>%
                mutate(
                  across(
                    c(
                  ...
                  re_still_limits_dispatch = coalesce(
                    re_still_limits_dispatch,
                    TRUE
                  ),
                ) %>%
                filter(
                  re_energy <= (cur_build_year - (irp_year - 1)) / 5,
                  re_energy >= re_energy_max,
                  storage_li_pct >= storage_li_pct_max,
                  (re_still_limits_dispatch == re_limits_dispatch) |
                    !re_limits_dispatch,
              ...
              final_data_inputs <- selectable_scenarios %>%
                ...
                inner_join(
                  consolidated_final_data_inputs %>%
                    filter(operating_year > cur_build_year),
                  by = c(
                    "Utility_ID_Econ",
                    "ba_code",
                    "scenario"
              ...
              final_fossil_costs <- rbind(
                final_fossil_costs,
                fossil_selected_years
              )
              ...
              final_data_inputs <- final_data_inputs %>%
                mutate(
                  ...
                  MWh_PTC = MWh,
                  owned_redispatch_mmbtu = owned_redispatch_mmbtu *
                    case_when(
                      ccs_eligible & category == "patio_clean" ~
                        CCS_Parasitic_Load, # incremental fuel use with CCS
                      category == "proposed_fossil" |
                        category == "proposed_clean" ~
                        1,
                      TRUE ~ 0
                    ),
                  owned_redispatch_co2_tonne = owned_redispatch_co2_tonne *
                    case_when(
                      ccs_eligible & category == "patio_clean" ~
                        CCS_Parasitic_Load +
                          (1 + CCS_Parasitic_Load) * (-CCS_CO2_Frac), # change in CO2 emissions with CCS
                      category == "proposed_fossil" |
                        category == "proposed_clean" ~
                        1,
                      TRUE ~ 0
                  ...
                  owned_co2_reduced = case_when(
                    ccs_eligible & category == "patio_clean" ~
                      -owned_redispatch_co2_tonne, # change in CO2 emissions with CCS
                    TRUE ~ owned_co2_reduced
                  ),
                  owned_redispatch_cost_fuel = owned_redispatch_cost_fuel *
                    case_when(
                      ccs_eligible & category == "patio_clean" ~
                        CCS_Parasitic_Load, # incremental fuel costs with CCS
                      category == "proposed_fossil" |
                        category == "proposed_clean" ~
                        1,
                      TRUE ~ 0
                    ),
                  owned_redispatch_cost_startup = owned_redispatch_cost_startup *
                    case_when(
                      ccs_eligible & category == "patio_clean" ~
                        0 * CCS_Parasitic_Load, # we assume no incremental startup costs with CCS
                      category == "proposed_fossil" |
                        category == "proposed_clean" ~
                        1,
                      TRUE ~ 0
                    ),
                  owned_stored_co2_tonne = owned_redispatch_co2_tonne *
                    case_when(
                      ccs_eligible & category == "patio_clean" ~
                        (1 + CCS_Parasitic_Load) * CCS_CO2_Frac, # stored CO2 emissions with CCS
                      category == "proposed_fossil" |
                        category == "proposed_clean" ~
                        1,
                      TRUE ~ 0
              ...
              final_data_inputs <- final_data_inputs %>%
                mutate(
                  MW = if_else(
                    (!is.na(NREL_Class)) & (!is.na(CF_improvement)),
                    MW / CF_improvement,
                    MW
                  ),
                  Clean_CapEx = (1 + supply_chain_tariff_CAPEX_impact) *
                    CAPEX *
                    MW *
                    1000,
                  cf_actual = case_when(
                    operating_year %% 4 == 0 & MW > 0 ~ (MWh / (MW * 8784)),
                    MW > 0 ~ (MWh / (MW * 8760)),
                    TRUE ~ NA
                  )
                ) %>%
              ...
              final_data_inputs <- final_data_inputs %>%
                mutate(
                  Fuel_Costs = (owned_redispatch_cost_fuel +
                    Fuel *
                      cf_actual *
                      (8.76 + (operating_year %% 4 == 0) * 0.024) *
                      MW *
                      1000) *
                    Inflator,
                  VOM = `Variable O&M` *
                  ...
                  Clean_PTC = -can_claim_credit *
                    ...
                    if_else(
                      transferability_eligible,
                      Transferability_Discount,
                      1
                    ),
                  Clean_45Q = -(can_claim_credit *
                    ccs_eligible *
                    (cur_project_year <= Duration_45Q)) *
                    if_else(
                      transferability_eligible & cur_project_year > 5,
                      Transferability_Discount,
                      1
                    ) *
                    ...
                    round(
                      Value_45Q *
                        if_else(
                          operating_year >= Inflation_Start_Year_45Q,
                          Inflation_Factor_2021 / Inflator_2025,
                          1
                        )
                    )
                )
              ...
              final_data_inputs <- final_data_inputs %>%
                group_by(index) %>%
                mutate(
                  ITC_Better = (-sum(Clean_PTC * Disc_Fin, na.rm = TRUE) <
                    ITC_reg * Clean_CapEx) &
                    (ITC_reg > 0)
                ) %>%
                ungroup()
              ...
              utility_ba_co2_reductions <- owned_co2_reductions %>%
                ...
                group_by(Utility_ID_Econ, ba_code) %>%
                mutate(
                  co2_red_selected = sum(
                    co2_red *
                      is_selected *
                      (year > irp_year & year <= cur_build_year),
                    na.rm = TRUE
                  )
                ) %>%
                ungroup() %>%
                ...
                mutate(
                  co2_red = sum(
                    co2_red *
                      (year > irp_year & year <= irp_year + NPV_Duration),
                    na.rm = TRUE
                  ) +
                    co2_red_selected
                ) %>%
                ungroup() %>%
                select(-c(co2_red_selected, is_selected, year)) %>%
                distinct() %>%
              ...
              coop_public_capital_limits <- final_data_inputs %>%
                ...
                summarize(
                  ITC_Required = sum(
                    Clean_CapEx * ITC * ITC_Better,
                    na.rm = TRUE
                  ),
                  ITC_Available = sum(Clean_CapEx * ITC, na.rm = TRUE) -
                    ITC_Required,
                ...
                mutate(
                  ...
                  NewERA = if_else(
                    ...
                    pmin(
                      ...
                      0.25 * coalesce(Clean_CapEx, 0)
                    ),
                    0
                  ),
                  NewERA_Deficit = (New_ERA_Total *
                    10^9 -
                    sum(NewERA, na.rm = TRUE))
                ) %>%
                ungroup()
              ...
              for (i in 1:20) {
                coop_public_capital_limits <- coop_public_capital_limits %>%
                  ...
                  mutate(
                    NewERA = if_else(
                      (NewERA_Deficit > 10^7) & (co2_red >= 100) & coop,
                      pmin(
                        New_ERA_Total * 10^8,
                        (New_ERA_Total * 10^9 + NewERA_Deficit) *
                          co2_red /
                          tot_co2_red,
                        0.25 * coalesce(Clean_CapEx, 0)
                      ),
                    ...
                    NewERA_Deficit = coop *
                      (New_ERA_Total * 10^9 - sum(NewERA, na.rm = TRUE))
                  ) %>%
                  ungroup()
              }
              ...
              coop_public_capital_limits <- coop_public_capital_limits %>%
                mutate(
                  NewERA_per_tonne = if_else(
                    coop & (co2_red > 0),
                    NewERA / co2_red,
                    0
                  ),
                  NewERA_award = if_else(
                    (co2_red > 0) & coop,
                    pmin(
                      New_ERA_Total * 10^8,
                  ...
                  NewERA_Frac = if_else(
                    ...
                    pmax(
                      0,
                      ((NewERA_award / Clean_CapEx) - NewERA_subsidy_cost) /
                        (1 - NewERA_subsidy_cost)
                    ),
                    0
                  ),
                  NewERA_Debt_Frac = if_else(
                    coop & (Clean_CapEx > 0),
                    pmin(
                      1,
                      ((NewERA_award / Clean_CapEx) - NewERA_Frac) /
                        NewERA_subsidy_cost
                    ),
                    0
                  ),
                  equity_needs = (Min_Pub_Equity_Ratio * Clean_CapEx) -
                    if_else(
                      ...
                      0,
                      (coop_equity -
                        (coop_debt + coop_equity) * (Min_Pub_Equity_Ratio))
                    ) -
                    (NewERA_Frac * Clean_CapEx + ITC_Required),
                  ITC_Frac_Req = pmax(0, equity_needs / ITC_Available),
                  NewERA_Frac = if_else(
                    (coop & (Clean_CapEx > 0)) &
                      (is.na(ITC_Frac_Req) | ITC_Frac_Req > 1),
                    (NewERA_award / Clean_CapEx),
                    NewERA_Frac
                  ),
                  NewERA_Debt_Frac = if_else(
                    is.na(ITC_Frac_Req) | ITC_Frac_Req > 1,
                  ...
                  equity_needs = if_else(
                    is.na(ITC_Frac_Req) | ITC_Frac_Req > 1,
                    (Min_Pub_Equity_Ratio * Clean_CapEx) -
                      if_else(
                        is.na(coop_equity),
                        0,
                        (coop_equity -
              ...
              final_data_inputs <- final_data_inputs %>%
                ...
                mutate(
                  ...
                  NewERA_Debt_Frac = if_else(
                    (build_year >= irp_year) &
                      (build_year <= EIR_NewERA_fin_build_year) &
                      !is.na(NewERA_Debt_Frac),
                    NewERA_Debt_Frac,
                    0
                  ),
                  tax_equity_used = if_else(
                    category %in% c("patio_clean", "proposed_clean"),
                    (ITC_Frac_Req > 1) | tax_equity_used,
                    FALSE
                  )
                )
              ...
              final_data_inputs <- final_data_inputs %>%
                mutate(
                  Clean_PTC = if_else(ITC_Better, 0, (1 - ITC_Frac_Req)) *
                    Clean_PTC,
                  ...
                  debt_cost = case_when(
                    ...
                    (debt_spread == "BBB") & !(public_utility | coop) ~
                      BBB_Loan_Rates,
                    (debt_spread == "B") & !(public_utility | coop) ~
                      B_Loan_Rates,
                    (debt_spread == "BB") & !(public_utility | coop) ~
                      BB_Loan_Rates,
                    (debt_spread == "A") & !(public_utility | coop) ~
                      A_Loan_Rates,
                    (debt_spread == "AA") & !(public_utility | coop) ~
                      AA_Loan_Rates,
                    (debt_spread == "AAA") & !(public_utility | coop) ~
                      AAA_Loan_Rates,
                    TRUE ~ debt_cost
                  ),
                  ...
                  adjusted_ror = (roe * equity_ratio) +
                    ((debt_cost * (1 - equity_ratio)) * (1 - Blended_Tax_Rate)),
                  no_policy_ror = (roe * equity_ratio) +
                    no_policy_debt_cost *
                      (1 - equity_ratio) *
                      (1 - Blended_Tax_Rate),
                  Clean_EIR_OBS_Frac = case_when(
                    Clean_EIR_Off_BS == "Yes" &
                      iou &
                      build_year <= EIR_NewERA_fin_build_year ~
                      New_Asset_Debt_Replaced_by_EIR +
                        New_Asset_Equity_Replaced_by_EIR,
                    TRUE ~ 0
                  ),
                  doe_adjusted_ror = (roe *
                    (equity_ratio -
                      New_Asset_Equity_Replaced_by_EIR *
                        iou *
                        (build_year <= EIR_NewERA_fin_build_year))) +
                  ...
                  doe_adjusted_ror = case_when(
                    Clean_EIR_Off_BS == "Yes" &
                      iou &
                      (build_year <= EIR_NewERA_fin_build_year) ~
                      doe_adjusted_ror / (1 - Clean_EIR_OBS_Frac),
                    iou ~
                      doe_adjusted_ror +
                        (DOE_Loan_Rates *
                          (New_Asset_Equity_Replaced_by_EIR +
                  ...
                  Clean_EIR_PMT = Clean_EIR_OBS_Frac *
                    DOE_Loan_Rates /
                    (1 - (1 / (1 + DOE_Loan_Rates))^Debt_Tenor)
                )
              ...
              final_data_inputs <- final_data_inputs %>%
                mutate(
                  ...
                  cur_equity_ratio = utility_debt_year *
                    (equity_ratio -
                      New_Asset_Equity_Replaced_by_EIR *
                        iou *
                        (build_year <= EIR_NewERA_fin_build_year)) *
                    case_when(
                      Clean_EIR_Off_BS == "Yes" &
                        iou &
                        (build_year <= EIR_NewERA_fin_build_year) ~
                        1 / (1 - Clean_EIR_OBS_Frac),
                      TRUE ~ 1
                    ) +
                    (!utility_debt_year) * equity_ratio,
                  cur_ror = utility_debt_year *
                    doe_adjusted_ror +
                    (!utility_debt_year) * adjusted_ror
                ) %>%
              ...
              final_data_inputs <- final_data_inputs %>%
                mutate(
                  ...
                  Clean_ADIT = -Clean_CapEx *
                    ((cur_ror / (1 - Blended_Tax_Rate)) *
                      (sum_MACRS -
                        cur_project_year * (1 - ITC / 2) / Asset_Life) *
                      Blended_Tax_Rate),
                  ...
                  Clean_ITC_NewERA = -Clean_CapEx *
                    (ITC_reg / (1 - Blended_Tax_Rate) + NewERA_Frac) *
                    utility_itc_norm_year *
                    (1 /
                      ITC_Norm_Period +
                      cur_ror *
                        (!ITC_Normalized) *
                  ...
                  Clean_Rate_Base = Clean_CapEx *
                    ((1 - cur_project_year / Asset_Life) *
                      (1 - Clean_EIR_OBS_Frac) - # Clean capital costs
                      (sum_MACRS -
                        cur_project_year * (1 - ITC / 2) / Asset_Life) *
                        Blended_Tax_Rate - # ADIT
                      (ITC_reg * (!ITC_Normalized) + NewERA_Frac) *
                        utility_itc_norm_year * # Reducing rate base by ITC over asset life if not an IOU, and over voluntary normalization period if an IOU and ITC tax normalization rules do not apply
                        (1 - cur_project_year / ITC_Norm_Period)),
                  Clean_Pre_Tax_Earnings = Clean_Rate_Base *
                    cur_equity_ratio *
                    (roe / (1 - Blended_Tax_Rate)),
              ...
              final_data_inputs <- final_data_inputs %>%
                ...
                arrange(cur_project_year) %>%
                mutate(
                  Sum_Tax_Liabilities = cumsum(Clean_Pre_Tax_Earnings) *
                    Blended_Tax_Rate
                )
              ...
              final_data_inputs <- final_data_inputs %>%
                ...
                mutate(
                  ...
                  Clean_Tax_Asset = Clean_Net_Tax_Asset *
                    (cur_ror / (1 - Blended_Tax_Rate)),
                  Capex_Costs = (Clean_PTC + Clean_45Q) /
                    (1 - Blended_Tax_Rate) +
                    Clean_Cap_Costs +
                    Clean_ADIT +
                    Clean_ITC_NewERA +
              ...
              capital_recycled_by_scenario <- final_data_inputs %>%
                ...
                summarize(
                  Capital_Recycled = sum(Clean_CapEx * (1 - ITC), na.rm = TRUE)
                ) %>%
                ungroup() %>%
                mutate(build_year = operating_year - 1) %>%
                select(-c(operating_year))

              refi_parquet <- refinancing_parquet %>%
                inner_join(
                  capital_recycled_by_scenario,
                  by = c(
                    "Utility_ID",
                    "plant_id_eia",
                    "generator_id",
                ...
                filter(
                  Capital_Recycled >=
                    Securitization_Amount *
                      refinancing_capital_recycling_trigger
                ) %>%
              ...
              patio_econ <- final_data_inputs %>%
                ...
                summarize(
                  ...
                  across(
                    c(Earnings, Forward_Earnings),
                    ~ sum(.x * (Utility_ID_Econ == Utility_ID), na.rm = TRUE)
                  )
                ) %>%
                ungroup()
              ...
              if (cur_build_year == min(loop_years)) {
                final_counterfactual_outputs <- final_data_inputs %>%
                  ...
                  left_join(
                    Disc_Factors,
                    by = c("Utility_ID_Econ" = "Utility_ID", "operating_year")
                  ) %>%
                  mutate(
                    across(
                      c(
                ...
                counterfactual_econ_by_tech <- rbind(
                  fossil_econ %>% filter(scenario == "counterfactual"),
                  ...
                  mutate(
                    ...
                    across(
                      c(Costs, Capex_Costs, Opex, MWh),
                      ~ sum(.x * Disc, na.rm = TRUE),
                      .names = "{.col}_Disc_Sum"
                    ),
                    across(
                      c(Earnings),
                      ~ sum(.x * Disc_E, na.rm = TRUE),
                      .names = "{.col}_Disc_Sum"
                    )
                  ) %>%
                  ungroup()
                ...
                counterfactual_econ <- counterfactual_econ_by_tech %>%
                  ...
                  summarize(
                    ...
                    across(
                      c(Forward_Earnings, Costs, Capex_Costs, Opex),
                      ~ sum(
                        .x * (operating_year == irp_year + 1),
                        na.rm = TRUE
                      ),
                      .names = "{.col}_irp_year"
                    ),
                    across(
                      c(Emissions, Emissions_Reduced),
                      ~ sum(
                        .x * (operating_year == target_build_year + 1),
                        na.rm = TRUE
                      ),
                      .names = "{.col}_fin_build_year"
                    ),
                    MW = max(MW, na.rm = TRUE)
                  ) %>%
                  ungroup() %>%
                  mutate(
                    Levelized_Costs = round(
                      if_else(
                        MWh_Disc_Sum > 0,
                        Costs_Disc_Sum / MWh_Disc_Sum,
                        Costs_Disc_Sum
                      ),
                      digits = 1
                    ),
              ...
              final_econ_by_category <- final_econ_by_tech %>%
                ...
                summarize(
                  ...
                  across(
                    c(Forward_Earnings, Costs, Capex_Costs, Opex),
                    ~ sum(.x * (operating_year == irp_year + 1), na.rm = TRUE),
                    .names = "{.col}_irp_year"
                  ),
                  across(
                    c(Emissions, Emissions_Reduced),
              ...
              final_econ <- final_econ_by_tech %>%
                    ...
                    Earnings_Disc *
                      (category %in%
                        c(
                          "patio_clean",
                          "transmission_lines",
                          "patio_transmission_upgrades"
                        )),
                    na.rm = TRUE
                  ) >
                    0),
                  net_patio_earnings = (sum(
                    Earnings_Disc *
                      (category %in%
                        c(
                          "patio_clean",
                ...
                mutate(
                  Levelized_Costs = round(
                    if_else(MWh_Disc_Sum > 0, Costs_Disc_Sum / MWh_Disc_Sum, 0),
                    digits = 1
                  ),
                  counterfactual_baseline = FALSE
                )
              ...
              final_econ <- rbind(final_econ, counterfactual_econ) %>%
                ...
                mutate(
                  prev_is_selected = coalesce(
                    prev_is_selected & !counterfactual_baseline,
                    FALSE
                  ),
                  Cost_Reduction = sum(
                    Levelized_Costs * counterfactual_baseline,
                    na.rm = TRUE
                  ) -
                    Levelized_Costs,
                  Savings_Fraction = Cost_Reduction /
                    sum(
                      Levelized_Costs * counterfactual_baseline,
                      na.rm = TRUE
                    ),
                  Min_Costs = ((min(
                    if_else(counterfactual_baseline, NA, Levelized_Costs),
                    na.rm = TRUE
                  )) ==
                    Levelized_Costs),
                  Max_Earnings = (Earnings_Disc_Sum ==
                    max(
                      Earnings_Disc_Sum *
                        ((Savings_Fraction >= (utility_savings_overshoot - 1)) |
                          Min_Costs) *
                  ...
                  Max_Earnings_Ties = (sum(
                    Max_Earnings & !counterfactual_baseline,
                    na.rm = TRUE
                  ) >
                    1),
                  Max_Cost_Reduction_Ties = (sum(
                    Max_Earnings_Ties &
                      Max_Cost_Reduction &
                      !counterfactual_baseline,
                    na.rm = TRUE
                ...
                mutate(
                  Min_Feasible = Max_Earnings &
                    Max_Cost_Reduction &
                    (re_energy == min(re_energy, na.rm = TRUE)) &
                    (storage_li_pct == min(storage_li_pct, na.rm = TRUE)) &
                    (nuclear_scen == min(nuclear_scen, na.rm = TRUE)) &
                    (ccs_scen == min(ccs_scen, na.rm = TRUE)) &
                    !counterfactual_baseline,
                  is_selected = Max_Earnings &
                    ((!Max_Earnings_Ties) |
                      (Max_Cost_Reduction & Max_Earnings_Ties)) &
                    ((!Max_Cost_Reduction_Ties) |
                      (Min_Feasible & Max_Cost_Reduction_Ties)) &
                    !counterfactual_baseline,
                ...
                mutate(
                  ...
                  is_selected = case_when(
                    tot_sel == 0 & tot_prev_sel == 0 ~
                      (scenario == "counterfactual") & !counterfactual_baseline,
                  ...
                  re_still_limits_dispatch = as.logical(min(
                    if_else(is_selected, re_limits_dispatch, TRUE),
                    na.rm = TRUE
                  )),
              ...
              final_econ_ba <- final_econ %>%
                ...
                summarize(
                  ...
                  ba_Earnings_Challenge = coalesce(
                    (sum(
                      Costs_Disc_Sum * (patio_earnings & net_patio_earnings),
                      na.rm = TRUE
                    ) <
                      ba_earnings_threshold *
                        sum(Costs_Disc_Sum * patio_earnings, na.rm = TRUE)),
                    FALSE
                  )
                ) %>%
                ungroup() %>%
                ...
                mutate(
                  across(
                    c(ba_Costs_Disc_Sum, ba_MWh_Disc_Sum, ba_Earnings_Disc_Sum),
                    ~ sum(.x * counterfactual_baseline, na.rm = TRUE),
                    .names = "{.col}_count"
                  ),
                  ba_Levelized_Costs_count = if_else(
                    pmin(ba_MWh_Disc_Sum_count, ba_MWh_Disc_Sum) > 0,
                    ba_Costs_Disc_Sum_count /
                      pmin(ba_MWh_Disc_Sum_count, ba_MWh_Disc_Sum),
                    NA
                  ),
                  ba_Levelized_Costs = if_else(
                    pmin(ba_MWh_Disc_Sum_count, ba_MWh_Disc_Sum) > 0,
                    ba_Costs_Disc_Sum /
                      pmin(ba_MWh_Disc_Sum_count, ba_MWh_Disc_Sum),
                    NA
                  ),
                  ba_Cost_Reduction = ba_Levelized_Costs_count -
                    ba_Levelized_Costs,
                  ba_Min_Costs = coalesce(
                    (scenario[which.max(if_else(
                      counterfactual_baseline,
                      -Inf,
                      ba_Cost_Reduction
                    ))] ==
                  ...
                  ba_Max_Earnings = coalesce(
                    (scenario[which.max(if_else(
                      counterfactual_baseline |
                        ba_Earnings_Challenge |
                        (coalesce(
                          ba_Cost_Reduction < ba_savings_tolerance,
                          FALSE
                        ) &
                          !ba_Min_Costs),
                      -Inf,
                      ba_Earnings_Disc_Sum
                    ))] ==
                      scenario) &
                      !counterfactual_baseline,
                    FALSE
                  ),
                  ba_is_selected = ((ba_least_cost & ba_Min_Costs) |
                    (ba_Max_Earnings & !ba_least_cost)) &
                    !counterfactual_baseline,
              ...
              final_econ <- final_econ %>%
                left_join(
                  final_econ_ba,
                  by = c("ba_code", "scenario", "counterfactual_baseline")
                ) %>%
                group_by(Utility_ID_Econ, ba_code) %>%
                mutate(
                  ba_still_counterfactual = sum(
                    ba_is_selected * (scenario == "counterfactual"),
                    na.rm = TRUE
                  ) >
                    0,
                  utility_wants_to_build = sum(
                    is_selected * (scenario != "counterfactual"),
                    na.rm = TRUE
                  ) >
                    0,
                  ba_tot_sel = sum(ba_is_selected, na.rm = TRUE),
                  ba_is_selected = case_when(
                    utility_override_for_counterfactual_ba &
                      ba_tot_sel == 0 &
                      tot_sel == 0 &
                      tot_prev_sel == 0 ~
                      (scenario == "counterfactual") & !counterfactual_baseline,
                    ...
                    utility_override_for_counterfactual_ba &
                      ba_tot_sel > 0 &
                      ba_still_counterfactual &
                      utility_wants_to_build ~
                      is_selected,
                    ba_tot_sel == 0 & tot_prev_sel > 0 ~ prev_is_selected,
                    ba_tot_sel == 0 & tot_prev_sel == 0 ~
                      (scenario == "counterfactual") & !counterfactual_baseline,
                    TRUE ~ ba_is_selected
                  ),
                  ba_tot_sel = sum(ba_is_selected, na.rm = TRUE),
                  ...
                  ba_re_still_limits_dispatch = as.logical(min(
                    if_else(ba_is_selected, re_limits_dispatch, TRUE),
                    na.rm = TRUE
                  )),
              ...
              if (ba_scenario_selection) {
                cur_scenario_selected <- selectable_scenarios %>%
                  ...
                  inner_join(
                    final_econ %>%
                      filter(ba_is_selected) %>%
                      filter(!counterfactual_baseline) %>%
                      select(
                        Utility_ID_Econ,
                        ba_code,
                        scenario,
                        is_selected = ba_is_selected,
            ...
            additional_years <- c(
              (max(loop_years) + 2):(irp_year + NPV_Duration)
            )

            scenarios_selected_by_year <- rbind(
              scenario_selected %>%
                select(
                  Utility_ID_Econ,
                  ba_code,
            ...
            if (ba_scenario_selection) {
              scenarios_selected_by_year_ba <- scenarios_selected_by_year %>%
                select(-c(Utility_ID_Econ)) %>%
                distinct()

              final_fossil_outputs <- fossil_parquet %>%
                left_join(
                  scenarios_selected_by_year_ba,
                  by = c("ba_code", "scenario", "operating_year")
                ) %>%
              ...
              final_fossil_outputs <- fossil_parquet %>%
                ...
                left_join(
                  scenarios_selected_by_year_ba %>%
                    rename(is_selected_ba = is_selected),
                  by = c("ba_code", "scenario", "operating_year")
                ) %>%
                mutate(is_selected = coalesce(is_selected, is_selected_ba)) %>%
                select(-c(is_selected_ba)) %>%
                filter(is_selected == TRUE) %>%
                mutate(
                  counterfactual_baseline = FALSE,
                  historical_actuals = FALSE,
            ...
            final_outputs <- rbind(
              final_prop_and_clean_outputs %>%
                select(all_of(final_outputs_columns)),
              final_fossil_outputs %>% select(all_of(final_outputs_columns)),
              final_refinancing_outputs %>%
                select(all_of(final_outputs_columns))
            ) %>%
              mutate(
                technology_description = as.factor(technology_description),
...
cloud$write_cloud_file(
  final_outputs,
  paste("patio-results", patio_results, "final_outputs.parquet", sep = "/")
)
cloud$write_cloud_file(
  scenario_selected,
...
final_outputs_by_plant <- final_outputs_agg %>%
  ...
  summarize(
    across(
      c(MWh, Capex_Costs, Fuel_Costs, FOM, VOM, Startup_Costs, Opex, Costs),
      ~ sum(. * Disc * (operating_year %in% irp_years), na.rm = TRUE),
      .names = "{.col}_Disc_Sum"
    ),
    MW = sum(
      MW *
        (Technology_FERC != "transmission") *
        (operating_year == year_to_test),
      na.rm = TRUE
    ),
    across(
      c(
      ...
      ~ sum(. * (operating_year == year_to_test), na.rm = TRUE)
    ),
    Cumulative_Emissions = sum(
      Emissions * (operating_year <= year_to_test),
      na.rm = TRUE
    ),
    ...
    across(
      c(
        ...
        FOM_Disc_Sum,
        VOM_Disc_Sum,
        Startup_Costs_Disc_Sum,
        Opex_Disc_Sum
      ),
      ~ if_else(MWh_Disc_Sum > 0, . / MWh_Disc_Sum, NA),
      .names = "L{.col}"
    ),
    CF = MWh / (MW * (8760 + if_else(year_to_test %% 4 == 0, 24, 0))),
    Counterfactual_Emissions_2021 = sum(
      Emissions * (operating_year == 2021) * counterfactual_baseline,
      na.rm = TRUE
    ),
    Historical_Emissions_2021 = sum(
      Emissions * (operating_year == 2021) * historical_actuals,
      na.rm = TRUE
    )
  ) %>%
  ungroup()
...
final_outputs_by_ba_code <- final_outputs_agg %>%
  ...
  summarize(
    across(
      c(Costs, MWh, Capex_Costs, Fuel_Costs, FOM, VOM, Startup_Costs, Opex),
      ~ sum(
        . * Disc * (operating_year %in% irp_years) * (!counterfactual_baseline),
        na.rm = TRUE
      ),
      .names = "{.col}_Disc_Sum"
    ),
    across(
      ...
      .names = "{.col}_Disc_Sum_count"
    ),
    LCOE = if_else(MWh_Disc_Sum > 0, Costs_Disc_Sum / MWh_Disc_Sum, NA),
    LCOE_count = if_else(
      MWh_Disc_Sum_count > 0,
      Costs_Disc_Sum_count / MWh_Disc_Sum_count,
      NA
    ),
    LCOE_Diff = LCOE_count - LCOE,
    across(
      c(Costs, Capex_Costs, Fuel_Costs, FOM, VOM, Startup_Costs, Opex),
      ~ sum(
        . *
          Disc *
          (operating_year %in% irp_years) *
          (2 * counterfactual_baseline - 1),
        na.rm = TRUE
      ),
      .names = "{.col}_Disc_Sum_Diff"
    ),
    across(
      c(
        Costs,
...
file.remove(paste(cache_dir, "allocated_parquet.parquet", sep = "/"))
