...
def package_econ_data(run, ad, addl_filter, *, local=False):
    ...
    if local:
        proc = lambda file_: pl.scan_parquet(colo_dir / f"colo_{file_}.parquet")  # noqa: E731
    else:
        fs = rmi_cloud_fs()

        def proc(file_):
            with fs.open(f"patio-results/{run}/colo_{file_}.parquet") as f:
                return pl.scan_parquet(f)

    good = good_screen(pl.from_pandas(get_summary(run=run, ad=ad)).filter(addl_filter))
    selector = ~cs.by_dtype(pl.List(pl.Int64)) & ~cs.by_dtype(pl.Duration)
    for info in tqdm(set(Info.from_df(good))):
        for o_ids in good.filter(info.filter()).select("regime", "ix").iter_rows(named=True):
            ftsv_scr = info.filter(**o_ids)
            pdir = econ_dir / info.file(**o_ids, suffix="")
            pdir.mkdir(parents=True, exist_ok=True)
            for file in ("summary", "annual", "cost_detail", "flows", "re_selected", "full"):
                try:
                    proc(file).filter(ftsv_scr).select(selector).sink_csv(pdir / f"{file}.csv")
    ...
    #     "critical_hour",
    #     "load",
    #     pl.sum_horizontal(*model.re_types).alias("re"),
    #     "li_charge",
    #     "li_discharge",
    #     "li_soc",
    #     "load_fossil",
    ...
    #     "export_fossil",
    #     "export_requirement",
    #     pl.min_horizontal(pl.sum_horizontal("es", *model.re_types), "load").alias("agg"),
    #     "c_load",
    #     "c_load_fossil",
    #     "c_export_fossil",
    #     "c_export_clean",
    #     "c_curtailment",
    # ).collect()
    # bad = k.filter((pl.col("agg") < pl.col("load")) & (pl.col("export_clean") > 0))
    # z = (
    #     k.group_by(pl.col("datetime").dt.year().alias("year"))
    #     .agg(cs.numeric().sum())
    #     .with_columns(
    #         pct=1 - pl.col("load_fossil") / pl.col("load"),
    #         served=pl.col("load") / (175 * 8760),
    #     )
    #     .sort("year")
    # )
    ...
    #         # charge_=pl.sum_horizontal(cs.contains("_charge")),
    #         # discharge_=pl.sum_horizontal(cs.contains("discharge")),
    #         # charge=pl.sum_horizontal(cs.contains("_charge")),
    #         charge=-pl.min_horizontal(0.0, pl.sum_horizontal(cs.contains("discharge")) - pl.sum_horizontal(
    #             cs.contains("_charge"))),
    #         discharge=pl.max_horizontal(0.0, pl.sum_horizontal(cs.contains("discharge")) - pl.sum_horizontal(cs.contains("_charge"))),
    #         load_clean=c.load - c.load_fossil,
    #         load_re=pl.min_horizontal(
    #             c.load - c.load_fossil, pl.sum_horizontal(*model.re_types)
    #         ),
    #         export_req_clean=pl.min_horizontal(c.export_requirement, c.export_clean),
    #         export_addl_clean=c.export - c.export_requirement,
    #     )
    #     .with_columns(
    #         export_addl_re=pl.min_horizontal(c.export_addl_clean, c.re - c.load_re),
    #     )
    #     .with_columns(
    #         export_req_re=c.re
    #         - pl.sum_horizontal("export_addl_re", "load_re", "charge", "curtailment"),
    #     )
    #     .with_columns(
    #         export_req_storage=c.export_req_clean - c.export_req_re,
    #         export_req_fossil=c.export_requirement - c.export_req_clean,
    #         export_addl_storage=c.export_addl_clean - c.export_addl_re,
    #         load_storage=pl.min_horizontal(c.load - c.load_fossil - c.load_re, c.discharge),
    #         storage_re=pl.min_horizontal(c.re, c.charge),
    #         curtailment_re=c.curtailment,
    #     )
    #     # .with_columns(
    #     #     storage_storage=c.charge_ - c.storage_re,
    #     # )
    # )
    ...
    # flows = flows.with_columns(
    #     **{
    #         f"{part}_{t}": alloc[t] * pl.col(f"{part}_re")
    #         for part in to_repl
    #         for t in model.re_types
    #     }
    # )
    #
    # ch = flows.select(cs.ends_with("_charge")).collect()
    # ch_alloc = (ch / ch.sum_horizontal()).fill_nan(0.0)
    # dis = flows.select(cs.ends_with("discharge")).collect()
    # dis_alloc = (dis / dis.sum_horizontal()).fill_nan(0.0)
    #
    # flows = flows.with_columns(
    #     **{
    #         f"{t}_{re}": ch_alloc[f"{t}_charge"] * pl.col(f"storage_{re}")
    #         for re in (*model.re_types, "storage")
    #         for t in model.es_types
    #     },
    #     **{
    #         f"{part}_{t}": dis_alloc[f"{t}_discharge"] * pl.col(f"{part}_storage")
    #         for part in ("load", "export_req", "export_addl")
    #         for t in model.es_types
    #     },
    # )
    # if len(model.es_types) == 2:
    #     t0, t1 = model.es_types
    #     flows = flows.rename({f"{t0}_storage": f"{t0}_{t1}", f"{t1}_storage": f"{t1}_{t0}"})
    # else:
    #     t0, t1 = model.es_types[0], "es"
    #
    # flows = flows.select(
    #     ~cs.ends_with("_re")
    # & ~cs.ends_with("_storage")
    # & ~cs.starts_with("storage_")
    #     & ~cs.ends_with("_discharge")
    #     & ~cs.ends_with("_charge")
    #     & ~cs.ends_with("_clean")
    #     & ~cs.ends_with("_soc")
    ...
    #             c.load
    #             - pl.sum_horizontal(
    #             f"load_{t}" for t in ("fossil", "storage", *model.re_types)
    #
    #             # f"load_{t}" for t in ("fossil", *model.es_types, *model.re_types)
    #             )
    #         ).abs(),
    #         export_check=(
    #             c.export - pl.sum_horizontal(cs.matches("^export_req_.*|^export_addl_.*"))
    #         ).abs(),
    #         export_req_check=(
    #             c.export_requirement - pl.sum_horizontal(cs.matches("^export_req_.*"))
    #         ).abs(),
    #         fossil_check=(c.fossil - c.export_req_fossil - c.load_fossil).abs(),
    #         discharge_check=(c.discharge - pl.sum_horizontal(cs.matches(".*_storage$"))).abs(),
    #         charge_check=(c.charge - pl.sum_horizontal(cs.matches("^storage_.*"))).abs(),
    # discharge_check=(
    #     c.discharge_
    #     - pl.sum_horizontal(
    #         cs.matches("|".join(f".*_{x}$" for x in model.es_types))
    #         & ~cs.contains(f"{t0}_{t1}")
    #         & ~cs.contains(f"{t1}_{t0}")
    #     )
    # ).abs(),
    # charge_check=(
    #     c.charge_
    #     - pl.sum_horizontal(cs.matches("|".join(f"^{x}_.*" for x in model.es_types)))
    # ).abs(),
    # storage_check=(
    #         (c.discharge_ - c.charge_)
    #         - (
    #                 pl.sum_horizontal(cs.matches("|".join(f".*_{x}$" for x in model.es_types)))
    #                 - pl.sum_horizontal(
    #             cs.matches("|".join(f"^{x}_.*" for x in model.es_types))
    #         )
    #         )
    # ).abs(),
    #         curtailment_check=(
    #             c.curtailment - pl.sum_horizontal(cs.matches("^curtailment_.*"))
    #         ).abs(),
    #
    #     )
    #     .filter(
