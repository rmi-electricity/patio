...
from patio.model.colo_lp import (
    Data,
    Info,
    Model,
)
...
def model(test_dir, temp_dir, ix, pid, pad=""):
    shutil.copytree(
    ...
    i = Info(**PDATA[pid], years=(), max_re=5.0)
...
def test_pc(test_dir, temp_dir, ix, pid):
    pc = model(test_dir, temp_dir, ix, pid)
...
def test_pc_pure_surplus_form(copt_license_etc, test_dir, temp_dir, ix, pid):
    pc = model(test_dir, temp_dir, ix, pid, "_formsis")
...
def test_pc_pure_surplus(copt_license_etc, test_dir, temp_dir, ix, pid):
    pc = model(test_dir, temp_dir, ix, pid, "_fossis")
    FlexLoad(pc, uptime=0.5)
    Renewables(pc)
    EndogenousDurationStorage(pc)
    CleanExport(pc)
...
def test_storage(pc, meth, args, expected):
...
def test_renewables(pc, meth, args, expected):
    s = Renewables(pc)
...
def test_load(pc, meth, args, expected):
    s = EndogenousLoad(pc)
...
def test_curtailment(pc, meth, args, expected):
    s = Curtailment(pc)
...
def test_export(pc, meth, args, expected):
    s = CleanExport(pc)
...
def test_fossil_export(pc, meth, args, expected):
    s = IncumbentFossil(pc, pd.Series(25.0, index=DT_RNG))
...
def test_fossil_no_export(pc, meth, args, expected):
    s = IncumbentFossil(pc, pd.Series(25.0, index=DT_RNG), exportable=False)
...
def test_new_fossil(pc, meth, args, expected):
    s = LoadOnlyFossil(pc, pd.Series(25.0, index=DT_RNG), exportable=True)
...
def test_new_fossil_no_export(pc, meth, args, expected):
    s = LoadOnlyFossil(pc, pd.Series(25.0, index=DT_RNG), exportable=False)
